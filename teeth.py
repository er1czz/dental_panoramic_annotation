...
"""
""" 
1. original file modified to fit teeth segementation
2. will be modifying balloon.py to teeth.py 
"""
...
#                 polygons=polygons)
    ...
    def load_teeth(self, dataset_dir, subset):
        def pygn_extract(data_dir,json_name):
            print(json_name)
            a = json.load(open(os.path.join(data_dir,json_name)))
            plygns=[]
            for i in range(len(a['shapes'])):
                val = a['shapes'][i]
                dict_val = {}
                dict_val['name']='polygon'
                dict_val['all_points_x']=[int(points[0]) for points in val['points']]
                dict_val['all_points_y']=[int(points[1]) for points in val['points']]
                plygns.append(dict_val)
            return plygns,a['imagePath'],a['imageHeight'],a['imageWidth']
        # Add classes. We have only one class to add.
        self.add_class("teeth", 1, "teeth")
        ...
        jsons = [i for i in glob.glob(dataset_dir+"/*.json")]

        # Add images
        for a in jsons:
            try:
                polygons,image_name,height,width=pygn_extract(dataset_dir,a)
